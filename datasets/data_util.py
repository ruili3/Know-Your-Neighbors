...
def make_datasets(config):
    ...
    if type == "KITTI_Odometry":
        ...
        if debug_mode == False:
            ...
            split_postfix = "_debug"
        else:
            split_postfix = "_{}".format(debug_mode)
            
        train_dataset = KittiRawDataset(
            data_path=config["data_path"],
            pose_path=config["pose_path"],
            split_path=os.path.join(config["split_path"], "train_files{}.txt".format(split_postfix)),
        ...
        test_dataset = KittiRawDataset(
            data_path=config["data_path"],
            pose_path=config["pose_path"],
            split_path=os.path.join(config["split_path"], "val_files{}.txt".format(split_postfix)),
            target_image_size=config.get("image_size", (192, 640)),
            frame_count=config.get("data_fc", 1),
            return_stereo=config.get("data_stereo", False),
            keyframe_offset=config.get("keyframe_offset", 0),
        ...
        if config.get("split_path", None) is None:
            ...
            post_fix = "_debug" if config.get("debug", False) == True else "" 
            train_split_path = os.path.join(config["split_path"], "train_files{}.txt".format(post_fix))
            test_split_path = os.path.join(config["split_path"], "val_files{}.txt".format(post_fix))
...
def make_test_dataset(config):
    ...
    if type == "KITTI_Raw":
        test_dataset = KittiRawDataset(
            data_path=config["data_path"],
            pose_path=config["pose_path"],
            # debug in incomplete dataset
            split_path=os.path.join(config["split_path"], "test_files.txt" if 
                                        config.get("debug", False) == False else "test_files_debug.txt"),
            target_image_size=config.get("image_size", (192, 640)),
            return_depth=True,
            frame_count=1,
            return_stereo=config.get("data_stereo", False),
        ...
        test_dataset = Kitti360Dataset(
            data_path=config["data_path"],
            pose_path=config["pose_path"],
            split_path=os.path.join(config.get("split_path", None), "test_files{}.txt".format(split_postfix)),
            target_image_size=tuple(config.get("image_size", (192, 640))),
            frame_count=config.get("data_fc", 1),
            return_stereo=config.get("data_stereo", False),
