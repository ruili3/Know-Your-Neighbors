...
class Kitti360Dataset(Dataset):
    ...
    def _load_split(split_path, img_ids):
        # NOTE: i - 0~N sequential ids | id - ids in the pose_file 
        ...
        def split_line(l):
            segments = l.split(" ")
            seq = segments[0]
            # this id indicates the sequential 0-N ids
    ...
    def _load_calibs(data_path, fisheye_rotation=0):
        ...
        # print("init K:{}".format(K))
        # print("im_size_rect:{}".format(im_size_rect))
    ...
    def __getitem__(self, index: int):
        ...
        # id: id+2
        ids = [id] + [max(min(i, seq_len-1), 0) for i in range(id - self._left_offset, id - self._left_offset + self.frame_count * self.dilation, self.dilation) if i != id]
        # print("_left_offset:{}, frame_count:{}, dilation:{}".format(self._left_offset, self.frame_count, self.dilation))
        # print("ids:{}".format(ids))

        # print("self.fisheye_offset:{}".format(self.fisheye_offset))
        # NOTE default fisheye offset==10
        if not self.variant_fisheye_offset:
            ids_fish = [max(min(id + self.fisheye_offset, seq_len-1), 0)] + [max(min(i, seq_len-1), 0) for i in range(id + self.fisheye_offset - self._left_offset, id + self.fisheye_offset - self._left_offset + self.frame_count * self.dilation, self.dilation) if i != id + self.fisheye_offset]
        else:
            curr_fisheye_offset = random.randint(self.low_fisheye_offset, self.up_fisheye_offset)
            # print("curr_fisheye_offset:{}".format(curr_fisheye_offset))
            ids_fish = [max(min(id + curr_fisheye_offset, seq_len-1), 0)] + [max(min(i, seq_len-1), 0) for i in range(id + curr_fisheye_offset - self._left_offset, id + curr_fisheye_offset - self._left_offset + self.frame_count * self.dilation, self.dilation) if i != id + curr_fisheye_offset]  

        # print("ids_fish:{}".format(ids_fish))
        ...
        # print("img_ids_fish:{}".format(img_ids_fish))
