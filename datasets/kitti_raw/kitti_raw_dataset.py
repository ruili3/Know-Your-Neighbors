...
class KittiRawDataset(Dataset):
    ...
    def load_images(self, day, seq, ids, load_left, load_right):
        ...
        for id in ids:
            if load_left:
                img = cv2.cvtColor(cv2.imread(os.path.join(self.data_path, day, seq, "image_02", "data", f"{id:010d}.{self.img_ext}")), cv2.COLOR_BGR2RGB).astype(np.float32) / 255
                imgs_left += [img]

            if load_right:
                img = cv2.cvtColor(cv2.imread(os.path.join(self.data_path, day, seq, "image_03", "data", f"{id:010d}.{self.img_ext}")), cv2.COLOR_BGR2RGB).astype(np.float32) / 255
                imgs_right += [img]
