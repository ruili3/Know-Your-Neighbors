...
class Backprojection(nn.Module):
    def __init__(self, height, width):
        ...
        yy, xx = torch.meshgrid([torch.linspace(-1., 1., self.H), torch.linspace(-1., 1., self.W)])
        yy = yy.contiguous().view(-1)
        xx = xx.contiguous().view(-1)
        self.ones = nn.Parameter(torch.ones(1, 1, self.H * self.W, dtype=torch.float), requires_grad=False)
    ...
    def forward(self, depth, inv_K):
        ...
        cam_p_h = torch.cat([cam_p_euc, self.ones.expand(n, -1, -1)], 1)
        return cam_p_h
...
class SSIM(nn.Module):
    ...
    """
    def __init__(self, pad_reflection=True, gaussian_average=False, comp_mode=False, eval_mode=False):
...
def ssim(x, y, pad_reflection=True, gaussian_average=False, comp_mode=False, eval_mode=False, pad=True):
