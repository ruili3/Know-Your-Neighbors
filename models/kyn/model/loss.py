...
class ReconstructionLoss:
    def __init__(self, config, use_automasking=False) -> None:
        ...
        # NOTE for object regularization loss
        self.labmda_objreg_loss = config.get("labmda_objreg_loss", 0)
        if self.labmda_objreg_loss:
            self.effective_distance = config.get("effective_distance", 20.0)
            # self.objreg_loss = torch.nn.MSELoss(reduction="none")
    ...
    def __call__(self, data):
        with profiler.record_function("loss_computation"):
            ...
            # print("scale:{}".format(n_scales))
            # NOTE the n_scale==1 in practice
            for scale in range(n_scales):
                ...
                # NOTE for objreg loss
                if self.labmda_objreg_loss > 0:
                    # img_bs, ray_bs, 1
                    depths = coarse["depth"].unsqueeze(-1)
                    # img_bs, ray_bs, 1
                    sceneprior = coarse["sceneprior"]
                    # img_bs, 1, K
                    ray_samps = coarse["z_samps"]
                    # img_bs, ray_bs, K
                    sigmas = coarse["sigma_all"]
                    
                    img_bs, n_samples = ray_samps.shape[0], ray_samps.shape[-1]

                    # reshape the elements: img_bs, ray_bs, K
                    depths = depths.reshape(img_bs, -1, 1)
                    ray_samps = ray_samps.reshape(img_bs, -1, n_samples)

                    # depths:torch.Size([1, 64, 8, 8, 1]), ray_samps:torch.Size([1, 64, 8, 8, 64]), sceneprior:torch.Size([1, 4096, 64]), sigmas:torch.Size([1, 4096, 64])

                    print("depths:{}, ray_samps:{}, sceneprior:{}, sigmas:{}".format(depths.shape, ray_samps.shape, sceneprior.shape, sigmas.shape))

                    print("sceneprior ray:{}".format(sceneprior[0, 3000, :]))

                    # the invalid categorie are set to -1
                    valid_prior_index = (sceneprior != -1)
                    # img_bs, ray_bs, 1 the value are depth endings
                    obj_ending_pos = (depths + sceneprior) * valid_prior_index + 1e5 * (~valid_prior_index)
                    
                    # only compute for far objects that lack in supervision
                    valid_far_area = (ray_samps > self.effective_distance)

                    # only supress the values behind the object
                    bts_obj_area = (ray_samps > obj_ending_pos)

                    # final selected samples to supress the density values
                    selected_samples = valid_far_area * bts_obj_area

                    # no samples
                    if selected_samples.sum() == 0:
                        obj_sigma_loss = 0
                        no_valid_sample = True
                        print("no sample")
                    else:
                        obj_sigma_loss = sigmas[selected_samples].mean() * self.labmda_objreg_loss
        ...
        if self.labmda_objreg_loss > 0:
            loss_dict["loss_obj_sigma"] = obj_sigma_loss.item() if not no_valid_sample else 0
