...
class PatchRaySampler(RaySampler):
    ...
    def sample(self, images, poses, projs):
        ...
        images = images.permute(0, 1, 3, 4, 2) # NOTE b v c h w
        ...
        for n_ in range(n):
            focals = projs[n_, :, [0, 1], [0, 1]]
            centers = projs[n_, :, [0, 1], [2, 2]]
            # NOTE rays of each camera wrt the world coords
    ...
    def reconstruct(self, render_dict, channels=None):
        ...
        if "sem_logits" in coarse:
            c_sem_logits = coarse["sem_logits"]
            f_sem_logits = fine["sem_logits"]
            sem_ch = c_sem_logits.shape[-1]
            coarse["rgb"] = c_sem_logits.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, v, sem_ch)
            fine["rgb"] = f_sem_logits.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, v, sem_ch)
            
            print("patch sem logits size:{}".format(coarse["rgb"].shape))
...
class PartialImageRaySampler(RaySampler):
    '''
    generate whole-image rays but only keep the cropped area, used for evaluating 
    ray transformer
    '''
    def __init__(self, z_near, z_far, crop_h, crop_w, height=None, width=None, channels=3, norm_dir=True):
        self.z_near = z_near
        self.z_far = z_far
        self.height = height
        self.width = width
    ...
    def sample(self, images, poses, projs):
        ...
        for n_ in range(n):
            ...
            # (B, H, W, 8)
            rays = util.gen_rays(poses[n_].view(-1, 4, 4), self.width, self.height, focal=focals, c=centers, z_near=self.z_near, z_far=self.z_far, norm_dir=self.norm_dir)

            # (B, H', W', 8)
            rays = rays[:, self.crop_h[0]:self.crop_h[1], self.crop_w[0]:self.crop_w[1], :]
            
            rays = rays.reshape(-1, 8)

            all_rays.append(rays)

        all_rays = torch.stack(all_rays).squeeze(0)
        # print("all_rays:{}".format(all_rays.shape))
