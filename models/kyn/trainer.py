...
from models.kyn.model.image_processor import make_image_processor, RGBProcessor
from models.kyn.model.loss import ReconstructionLoss, compute_errors_l1ssim
from models.kyn.model.ray_sampler import ImageRaySampler, PatchRaySampler, RandomRaySampler
...
class KYNWrapper(nn.Module):
    def __init__(self, renderer, config, eval_nvs=False) -> None:
        ...
        print(f"mlp parameters: {sum(p.numel() for p in self.renderer.net.mlp_coarse.parameters())}")
    ...
    def forward(self, data):
        ...
        if self.training:
            # NOTE: ids_loss indicate the images to be reconstructed from ids_render images
        ...
        with profiler.record_function("trainer_sample-rays"):
            # [0, 3, 4, 7] or [1, 2, 5, 6]
            # RGB GT is sampled from the sampler
        ...
        if self.prediction_mode == "multiscale":
        ...
        else: # NOTE: prediction_mode - default
            with profiler.record_function("trainer_render"):
                # all_ray shape: B, ray_b, 8: {cam_cen, direction, z_near, z_far}
                # for depth in the `render_dict`: img_batch, ray_batch, 1
                render_dict = self.renderer(all_rays, want_weights=True, want_alphas=True, want_rgb_samps=True)
            ...
            with profiler.record_function("trainer_reconstruct"):
                # depth: (sb, rb, 1) -> (sb, patch_n, patch_size, patch_size)
                # img: (sb, rb, 3 * v) -> (sb, patch_n, patch_size, patch_size, v, 3)
                render_dict = sampler.reconstruct(render_dict)

            # list - dict
            # data coarse len:1 shape:dict_keys(['rgb', 'depth', 'invalid', 'weights', 'alphas', 'rgb_samps'])
            # data fine len:1 shape:dict_keys(['rgb', 'depth', 'invalid', 'weights', 'alphas', 'rgb_samps'])
            data["fine"].append(render_dict["fine"])
            data["coarse"].append(render_dict["coarse"])
            data["rgb_gt"] = render_dict["rgb_gt"]
            data["rays"] = render_dict["rays"]

            # print("[training] data_fine_0_depth:{}".format(data["fine"][0]["depth"].shape))

            # [training - patch ] b, c=64, 8, 8 | [testing - image size]
            # print("data coarse len:{} shape:{}".format(len(data["coarse"]), data["coarse"][0]["depth"].shape))
            # print("data fine len:{} shape:{}".format(len(data["coarse"]), data["fine"][0]["depth"].shape))
        ...
        if self.training is False:
            ...
            # None if remove the language model
            data["seg_pred"] = self.renderer.net.seg_mask

            # print(" data_fine_0_depth_0:{}".format(data["fine"][0]["depth"][0].shape))
            # print(" seg_pred:{}".format(data["seg_pred"][0].shape))
    ...
    def compute_depth_metrics(self, data):
        ...
        # print("depth pred shape:{}".format(depth_pred.shape))
        # print("depth_gt shape:{}".format(depth_gt.shape))
...
def initialize(config: dict, logger=None):
    # [NOTE] used for sigma/color computation
    arch = config["model_conf"].get("arch", "BTSNet")
    net = globals()[arch](config["model_conf"])
    # [NOTE] used for per-pixel rendering
    renderer = NeRFRenderer.from_conf(config["renderer"])
    renderer = renderer.bind_parallel(net, gpus=None).eval()

    mode = config.get("mode", "depth")
    # [NOTE] used for whole image render
    model = KYNWrapper(
        renderer,
        config["model_conf"],
        mode == "nvs"
...
def visualize(engine: Engine, logger: TensorboardLogger, step: int, tag: str):
    ...
    # print("vis recon depth:{}".format(recon_depths[0].shape))
    ...
    # write sem maps
    if data["seg_pred"] is not None:
        vl_seg_mask = data["seg_pred"][0,0,0].cpu().numpy()
        # TODO: remove the fixed labels
        new_palette = get_new_pallete(21)
        seg_color = get_new_mask_pallete(vl_seg_mask, new_palette)
        writer.add_image(f"{tag}/vlseg_mask", seg_color, global_step=step)
