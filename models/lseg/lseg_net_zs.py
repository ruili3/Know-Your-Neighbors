...
class BaseModel(torch.nn.Module):
    def load(self, path):
        ...
        Args:
            path (str): file path
        """
        # [NOTE] customized for merely loading the weights
        parameters = torch.load(path, map_location=torch.device("cpu"))["state_dict"]
        parameters = {k.replace('net.', ''): v for k, v in parameters.items()}
...
class LSegRN(BaseModel):
    ...
    def forward(self, x, labelset=None):
        ...
        # [NOTE] change the code to have similar procedure of 
        texts = self.texts

        # print("texts:{}".format(len(texts)))
        # print("text [0]:{}".format(texts[0].shape))
        ...
        # print("layer_1_rn shape:{}".format(layer_1_rn.shape))
        # print("layer_2_rn shape:{}".format(layer_2_rn.shape))
        # print("layer_3_rn shape:{}".format(layer_3_rn.shape))
        # print("layer_4_rn shape:{}".format(layer_4_rn.shape))
        ...
        # print("image feat:{}".format(len(image_features)))
        # print("text_features feat:{}".format(len(text_features)))

        logits_per_images = [self.logit_scale * image_feature.half() @ text_feature.t() for image_feature, text_feature in zip(image_features, text_features)]

        # print("logits_per_images:{}".format(len(logits_per_images)))
...
class LSegRNMultiClass(BaseModel):
    def __init__(
        self,
        head,
        features=256,
        ...
        **kwargs,
        ...
        super(LSegRNMultiClass, self).__init__()
...
class LSegRNNetZSMultiClass(LSegRNMultiClass):
    """Network for semantic segmentation."""
    def __init__(self, label_list, path=None, scale_factor=0.5, aux=False, use_relabeled=False, use_pretrained=True, **kwargs):
