...
def base_training(local_rank, config, get_dataflow, initialize, get_metrics, visualize):
    ...
    if "save_best" in config:
        ...
        # [NOTE] sign in {-1, 1}, -1: smaller is better. Default in config: -1 (absrel)
        ...
        # [NOTE] to be executed when the event Events.COMPLETED is triggered and the condition lambda *_: trainer.state.epoch > config["num_epochs"] // 2 is met.
        evaluator.add_event_handler(
            Events.COMPLETED(lambda *_: trainer.state.epoch > config["num_epochs"] // 2), best_model_handler
        )

        # NOTE if save per-epoch model
        if config["save_best"].get("per_epoch_save", False):
            # Create a checkpoint handler to save the model at every epoch
            every_epoch_checkpoint_handler = Checkpoint(
                to_save={"model": model, "optimizer": optimizer},  # Objects to save
                save_handler=get_save_handler(config),  # Assuming you have a function get_save_handler to determine where to save
                filename_prefix="epoch",
                n_saved=None,  # set to None to keep all checkpoints
                global_step_transform=global_step_from_engine(trainer),
            )

            # Attach the handler to the trainer
            trainer.add_event_handler(Events.EPOCH_COMPLETED, every_epoch_checkpoint_handler)

            # print("save model for each epoch!")
...
def create_trainer(model, optimizer, criterion, lr_scheduler, train_sampler, config, logger, metrics={}):
    ...
    # [NOTE] the scaler factor is used to handle out-of-lower-bound gradient caused by amp-float16 precision
    # it scales up the gradient during back-propagation
...
def get_save_handler(config):
    # DiskSaver is a build-in module ensures "save all or change nothing (when exception happens)"
